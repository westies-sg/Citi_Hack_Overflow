...
export default function Home() {
  ...
  const intraBotQuery = () => {
    ...
    fetch('/api/intra', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ query: intraMessage, history: intraHistory }),
    })
      .then(async (res) => {
        const r = await res.json();
                      ...
                      <div className='flex sticky bottom-0 w-full px-6 pb-6 h-24'>
                        <div className='w-full relative'>
                          ...
                          <button
                            ...
                            type='submit'
                            aria-label='Send'
                            disabled={!message || publicBotLoading}
                          >
                            <Send />
                          </button>
                        </div>
              ...
              <div className='p-6 flex'>
                ...
                <div className='opacity-0 animate-fadeIn w-4/5 h-96 bg-white rounded-xl shadow-lg flex flex-col'>
                  ...
                  <div className='flex flex-col gap-8 w-full items-center flex-grow max-h-full '>
                    ...
                    >
                      <div className='no-scrollbar overflow-y-auto flex flex-col gap-5 p-10 h-full'>
                        {intraHistory.map((message: Message, idx) => {
                          const isLastMessage = idx === history.length - 1;
                          switch (message.role) {
                            case 'assistant':
                              return (
                                <div
                                  ref={
                                    isLastMessage ? intraLastMessageRef : null
                                  }
                                  key={idx}
                                  className='flex gap-2'
                                >
                        ...
                        {intraBotLoading && (
                          <div ref={intraLastMessageRef} className='flex gap-2'>
                            <img
                              src='images/citi_bot.jpg'
                              className='h-24 w-24 rounded-full border-blue'
                            />
                      ...
                      <div className='flex sticky bottom-0 w-full px-6 pb-6 h-24'>
                        <div className='w-full relative'>
                          <textarea
                            aria-label='chat input'
                            value={intraMessage}
                            onChange={(e) => setIntraMessage(e.target.value)}
                            placeholder='Type a message'
                          ...
                          <button
                            ...
                            type='submit'
                            aria-label='Send'
                            disabled={!intraMessage || publicBotLoading}
                          >
                            <Send />
                          </button>
                        </div>
